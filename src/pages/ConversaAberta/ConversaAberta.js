...
function ConversaAberta({ navigation, route, socket }) {
  ...
  useEffect(() => {
    ...
    async function enviaPdfS3(uri, nome){

      const chave = `PDF-${(Math.random() * 100).toString()}-${nome}`
      ...
      const options = {
        bucket: 'doctorappweb',
        region: 'sa-east-1',
        accessKey: 'AKIAY5CQE6QEP5DK55HE',
        secretKey: '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',
  ...
  useEffect(() => {
    ...
    async function enviarMensagemComMidia(Mediatype) {
      ...
      if (Mediatype === "Imagem") {
        ...
        urlS3 = await enviaPdfS3(arquivo, nomeArquivo)
        texto = "Arquivo PDF";
      } else return;
