...
function FormularioEmergencia({ navigation }) {
  ...
  async function GetFormulariosEmergenciaPaciente() {
    ...
    formularios.forEach((formularioPaciente) => {
      if (
        (formularioPaciente.id_formulario ===
          "046975f7-d7d0-4635-a9d9-25efbe65d7b7") &&
        (formularioPaciente.status === false)
      ) {
  ...
  async function pegandoDadosPaciente() {
    setCarregando(true);
  ...
  async function requisicaoEnviandoRespostas(respostas) {
    ...
    for (const pergunta in schema.properties) {
      if (respostas[pergunta] === undefined) campo_vazio = false;
    }
    if (campo_vazio) {
  ...
  return (
    <Container>
      ...
      <Corpo height={heightCorpo}>
        <CaixaCima>
          <CaixaTitulo>
            <Titulo fontSize="24px" color={Cores.azulEscuro} marginBottom="4px">
